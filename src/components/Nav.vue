<script setup>
    ...
    import { useSearchProducts,useProductClassification } from '@/stores/products';
    const router=useRouter();
    const searchProducts=useSearchProducts(); //搜尋
    const productClassification=useProductClassification(); //分類

    //產品分類
    const sendClassification=function(classification){
...
<template>
    <nav class="d-flex align-items-center justify-content-between mb-0 nav-bar" style="height: 40px;">
        <ul class="d-flex align-items-center justify-content-flex-start list-unstyled mb-0">
            <li><a href="#" class="text-decoration-none text-white me-5 ms-3" @click.prevent="sendClassification('clothes')">衣褲鞋襪</a></li>
            <li><a href="#" class="text-decoration-none text-white me-5" @click.prevent="sendClassification('backpack')">各類背包</a></li>
            <li><a href="#" class="text-decoration-none text-white me-5" @click.prevent="sendClassification('outdoor_litems')">戶外生活</a></li>
            <li><a href="#" class="text-decoration-none text-white me-5" @click.prevent="sendClassification('equipment')">器材裝備</a></li>
        </ul>
